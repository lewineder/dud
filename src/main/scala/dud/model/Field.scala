...
case class Field(cells: Array[Array[Building]]) {
    def this(sx: Int, sy: Int, filling: Building) = this(Phantom(Array.tabulate(sx,sy) { (row, col) => filling}))
    ...
    override def toString = {
        ...
        val colNumber = 1.to(row).map(i => output(0 + 5*(i-1)) + "\n" + output(1 + 5*(i-1)) + "\n" + output(2+5*(i-1)) + "     " + i.toString + "\n" + output(3+5*(i-1))+ "\n" + output(4+5*(i-1)) + "\n")

        /*val rowNumber = for {i <- 1 to col
            } yield " "*5 + i.toString + " " * 6*/
        val rowNumber = 1.to(col).map(i => " " *5 + i.toString + " " *6)
